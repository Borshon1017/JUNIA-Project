...
// ─── SEARCH BAR (all inner pages — bilingual) ───
const SEARCH_DATA = [
...
if (searchBtn && !searchBtn._psInit) {
  searchBtn._psInit = true;
  ...
  function openSearch() {
    ...
    if (searchInput) {
      searchInput.placeholder = isEN() ? 'Search programmes, campus…' : 'Rechercher formations, campus…';
  ...
  function renderResults(query) {
    ...
    const label = q === '' ? (en ? 'Quick links' : 'Liens rapides') : (en ? 'Results' : 'Résultats');
  ...
  if (searchClear) searchClear.addEventListener('click', () => { if (searchInput.value === '') { closeSearch(); searchBtn.focus(); } else { searchInput.value = ''; searchInput.focus(); renderResults(''); } });
  if (searchInput) {
...
// ─── LANGUAGE TOGGLE (inner pages — bidirectional FR ↔ EN) ───
(function(){
  ...
  btn.addEventListener('click', function(){
    window.location.href = isEN ? '../' + page : 'en/' + page;
  });
